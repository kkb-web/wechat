<template>
  <div>
    <div class="ali-weixin-tip weixin-tip">
      <div class="weixin-tip-content">
        请在菜单中选择在浏览器中打开,
        <br/>以完成支付
      </div>
    </div>
    <div class="ali-weixin-tip-img weixin-tip-img"></div>
    <!--支付异常提示 by zqy-->
    <div v-show="showUserError">
      <div class="user-mask"></div>
      <div class="user-encounter-problems">
        <img :src="userImg" alt="img" class="user-img"/>
        <div style="text-align: center">
          <p class="user-name">{{nickName}}你好</p>
          <p class="user-tip">报名过程遇到问题了吗？不要担心!</p>
          <p class="user-tip">联系你的课程顾问，Ta会帮助你哦~</p>
        </div>
      </div>
    </div>
    <!--异常end-->
    ...
    export default {
        data() {
            return {
                ...
                nickName: getToken("username") == "" ? "亲爱的学员" : getToken("username"),
        ...
        mounted() {
            ...
            this.interval = setInterval(this.getOrderStatus, 4000);
        },
        methods: {
            //判断是安卓还是IOS
            initPage() {
                if (location.hash.indexOf("error") != -1) {
                    ...
                    let ua = navigator.userAgent.toLowerCase(),
                        tip = document.querySelector(".weixin-tip"),
                        tipImg = document.querySelector(".ali-weixin-tip-img"),
                        userTip = document.querySelector(".user-encounter-problems");
                    if (ua.indexOf("micromessenger") != -1) {
                        ...
                        if (
                            ...
                            tipImg.className = "ali-weixin-tip-img weixin-tip-img iphone";
                            userTip.style.marginTop = "-150px";
                        } else {
                            tipImg.className = "ali-weixin-tip-img weixin-tip-img android";
                            userTip.style.marginTop = "-240px";
                        }
                    } else {
                        location.href = decodeURIComponent(_AP.a_decode(this.orderUrl));
                    }
                }
            },
            getdepostStatus(type, deadlineTime, no) { // type : 0:  1:  2:
                if (type === 0) {
                    window.location.href = `${cashierLink()}bindsuccess?code=` + sessionStorage.getItem('courseCode') + '&channel=' + sessionStorage.getItem("channelCode");
                } else if (type === 1) {
            ...
            getOrderStatus() {
                ...
                // 定金子订单查询
                if (this.orderPartId) {
                    sendData.orderPartId = this.orderPartId
                }
                let _that = this
                urlGetOrderStatus(sendData).then(res => {
                    const queryData = {orderId: _that.orderNo}
                    if (_that.orderPartId) {
                        queryData.orderPartId = _that.orderPartId
                    }
                    //判断订单状态
                    if (res.data.code === 0) {
                        let resData = res.data.data;
                        _that.showUserError = false;
                        // clearInterval(_that.interval); 
                        const courseModel = getToken('courseModel')
                        debugger
                        if (parseInt(courseModel) === 1) {
                          if(resData.sellType === 1){
                            _that.lodNextPartString = typeof(resData.nextPart) === "object"?resData.nextPart.join(','):resData.nextPart;
                            if(_that.isFirst){
                              _that.nextPartString = typeof(resData.nextPart) === "object"?resData.nextPart.join(','):resData.nextPart;
                              _that.isFirst = false
                            }
                            if(_that.lodNextPartString !== _that.nextPartString || resData.nextPart.length === 0){
                              clearInterval(_that.interval);
                              window.location.href = `${cashierLink()}groupDepositSuccess?orderId=${_that.orderNo}`
                            }
                            _that.showUserError = true;
                            return true
                          }
                          clearInterval(_that.interval);
                          _that.getdepostStatus(resData.partType,resData.deadlineTime,resData.no)
                        } else {
                            let orderPartId = _that.orderPartId ? `orderPartId=${_that.orderPartId}` : ''
                            window.location.href = `${routeUrl()}paysuccess?orderId=${_that.orderNo}&${orderPartId}`
                            // _that.$router.push({
                            //     path: routeUrl() + "paysuccess",
                            //     query: queryData
                            // });
                        }
                    } else {
